...
`calibrate_camera()` takes in a list of calibration image filenames and then runs `find_chessboard_corners()` on each image. It returns the camera calibration matrix returned from `cv2.calibrateCamera`.
